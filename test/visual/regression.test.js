...
describe('Butterchurn Visual Regression Tests', () => {
  ...
  testCases.forEach(({ name, seedIndependent }) => {
    test(`${name} - comprehensive regression test`, async () => {
      ...
      try {
        ...
        const screenshot1 = await renderButterchurn(page, serverUrl, width, height, name, audioData, FRAMES_TO_RENDER, SEED1);
        ...
        const screenshot2 = await renderButterchurn(page, serverUrl, width, height, name, audioData, FRAMES_TO_RENDER, SEED2);
